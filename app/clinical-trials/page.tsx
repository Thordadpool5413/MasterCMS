...
function TrialCard({ trial }: { trial: ClinicalTrial }) {
  const statusVariant = (STATUS_COLORS[trial.status] ?? "secondary") as
    | "success"
    | "warning"
    | "secondary"
    | "default";
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-[hsl(var(--muted-foreground))]">
        {trial.sponsor && (
          <div className="col-span-2 truncate" title={trial.sponsor}>
            <span className="font-medium text-[hsl(var(--foreground))]">Sponsor: </span>
            {trial.sponsor}
          </div>
        )}
        {primaryLocation && (
          <div className="flex items-center gap-1 col-span-2 truncate">
            ...
            <span title={primaryLocation.facility}>
              {[primaryLocation.facility, primaryLocation.city, primaryLocation.state]
                .filter(Boolean)
                .join(", ")}
            </span>
            {trial.locations.length > 1 && (
              <span className="text-[hsl(var(--muted-foreground))]">
                +{trial.locations.length - 1} sites
              </span>
            )}
          </div>
        )}
        ...
        <div className="flex-1 min-w-52">
          ...
          >
            <Input
              ...
              placeholder="e.g. Heart Failure, Cancer, COPD"
              className="flex-1"
            />
      ...
      {results && !loading && (
        <>
          <p className="mb-4 text-xs text-[hsl(var(--muted-foreground))]">
            {results.total.toLocaleString()} trials found · Source: ClinicalTrials.gov (no API key required)
          </p>
          ...
          {results.trials.length === 0 ? (
            <EmptyState
              ...
              description="Try a different condition, remove the state filter, or change the status filter."
            />
          ) : (
