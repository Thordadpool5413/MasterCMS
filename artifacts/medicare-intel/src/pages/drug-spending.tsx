...
function AdverseEventsPanel({ drugName }: { drugName: string }) {
  ...
  async function load() {
    ...
    setLoading(true);
    try {
      const res = (await mcp("get_fda_adverse_events", { drug_name: drugName })) as { reactions: AdverseEventReaction[]; total_reports: number };
  ...
  if (!reactions.length) return <p className="text-sm text-[hsl(var(--muted-foreground))] py-4">No adverse event data found for {drugName}.</p>;

  const maxCount = reactions[0]?.count ?? 1;
  return (
    <div>
      <p className="mb-3 text-xs text-[hsl(var(--muted-foreground))]">{totalReports.toLocaleString()} total reports in FDA FAERS · Top reactions shown</p>
      <div className="space-y-1.5">
        {reactions.map((r) => (
          <div key={r.term} className="flex items-center gap-3">
            <div className="w-48 shrink-0">
              <div className="h-5 rounded bg-[hsl(var(--muted))] overflow-hidden">
                <div className="h-full rounded bg-amber-400 transition-all" style={{ width: `${(r.count / maxCount) * 100}%` }} />
              </div>
            </div>
            <span className="text-sm font-medium capitalize">{r.term.toLowerCase()}</span>
            <span className="ml-auto text-xs text-[hsl(var(--muted-foreground))] font-mono">{r.count.toLocaleString()}</span>
          </div>
        ))}
      </div>
      <p className="mt-3 text-xs text-[hsl(var(--muted-foreground))]">Source: FDA FAERS — voluntary adverse event reports. Does not imply causation.</p>
    </div>
  );
}
...
function DrugLabelPanel({ drugName }: { drugName: string }) {
  ...
  if (error) return <ErrorBanner message={error} />;
  if (!label) return <p className="text-sm text-[hsl(var(--muted-foreground))] py-4">No FDA label found for {drugName}.</p>;
...
export default function DrugSpendingPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      <PageHeader icon={DollarSign} title="Drug Spending" description="Medicare Part D and Part B spending trends — click any drug to see FDA adverse events and label data." />
            ...
            {(["part_d", "part_b"] as const).map((t) => (
              <button
                ...
                className={cn(
                  ...
                  spendingType === t ? "bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]" : "bg-[hsl(var(--background))] text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--accent))]",
                )}
              >
                ...
                {result.rows.map((row, i) => {
                  ...
                  return (
                    <>
                      <TableRow
                        key={key}
                        className={cn("cursor-pointer transition-colors", isOpen ? "bg-[hsl(var(--accent))]" : "hover:bg-[hsl(var(--accent)/0.5)]")}
                        onClick={() => toggle(key)}
                      >
                        <TableCell className="px-3">
                          {isOpen ? <ChevronUp className="h-4 w-4 text-[hsl(var(--muted-foreground))]" /> : <ChevronDown className="h-4 w-4 text-[hsl(var(--muted-foreground))]" />}
                        </TableCell>
                        ...
                        <TableCell className="text-right text-sm">
                          {row.year_2023_spending ? <Badge variant="secondary">{usd(row.year_2023_spending)}</Badge> : "—"}
                        </TableCell>
