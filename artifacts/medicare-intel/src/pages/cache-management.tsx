...
const datasetColumns: Column<CachedDataset>[] = [
  ...
  { key: "scope", label: "Scope" },
  {
    key: "cached_at",
    label: "Cached At",
    render: (v) => v ? new Date(String(v)).toLocaleString("en-US") : "—",
  },
];
  ...
  async function handleCacheCore() {
    ...
    try {
      ...
      const rows: CacheResultRow[] = Object.entries(data.results ?? {}).map(([key, val]) => ({
        ...
        details: val.error ?? (val.row_count != null ? `${val.row_count.toLocaleString("en-US")} rows` : "—"),
      }));
    ...
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      <Alert className="mb-6">
        <AlertDescription className="text-xs">
          All CMS data is fetched live — no local cache is currently implemented. This page shows the cache stub interface for future use.
        </AlertDescription>
      </Alert>
