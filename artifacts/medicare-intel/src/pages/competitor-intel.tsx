...
import { Search, Building, ChevronDown, ChevronUp, ExternalLink, FileText, TrendingUp, TrendingDown } from "lucide-react";
...
const SUGGESTIONS = ["VITAS", "Amedisys", "Enhabit", "Crossroads Hospice", "Agrace", "Seasons Hospice", "LHC Group", "Addus Homecare"];
...
function DetailPanel({ ein }: { ein: string }) {
  ...
  useState(() => {
    mcp("get_nonprofit_detail", { ein })
      ...
      .catch((e) => setError(e instanceof Error ? e.message : "Failed to load filing data"))
  ...
  return (
    <div className="bg-[hsl(var(--muted)/0.4)] border-t border-[hsl(var(--border))] px-4 py-4">
      ...
      {filings.length > 0 ? (
            ...
            <TableBody>
              {filings.map((f) => (
                <TableRow key={f.tax_prd_yr}>
                  ...
                  <TableCell className="text-right"><Margin revenue={f.totrevenue} expenses={f.totfuncexpns} /></TableCell>
                  <TableCell>
                    {f.pdf_url && (
                      <a href={f.pdf_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-[hsl(var(--primary))] hover:underline">
        ...
        <p className="text-sm text-[hsl(var(--muted-foreground))]">No structured filing data available.</p>
      )}
    </div>
  );
}
...
export default function CompetitorIntelPage() {
  ...
  async function handleSearch(q = query) {
    ...
    setExpanded(null);
    try {
      const data = (await mcp("search_nonprofits", { query: q.trim(), ...(state ? { state } : {}) })) as { organizations: NonprofitOrg[]; total: number };
      setResults(data);
    } catch (e) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      <PageHeader icon={Building} title="Competitor Intelligence" description="Search IRS Form 990 filings for any nonprofit hospice, home health, or palliative care organization." />
      ...
      <div className="mb-4 flex flex-wrap items-end gap-3">
        <div className="flex-1 min-w-56">
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleSearch(); }} className="flex gap-2">
            <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="e.g. VITAS, Amedisys, Crossroads" className="flex-1" />
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        {SUGGESTIONS.map((s) => (
          <button
            ...
            className="rounded-full border border-[hsl(var(--border))] px-3 py-1 text-xs text-[hsl(var(--muted-foreground))] hover:border-[hsl(var(--primary)/0.5)] hover:text-[hsl(var(--primary))] transition-colors"
          >
            {s}
          </button>
        ))}
      ...
      {results && !loading && (
        <>
          <p className="mb-3 text-xs text-[hsl(var(--muted-foreground))]">
            {results.total.toLocaleString()} organizations found{state ? ` in ${state}` : ""} · Source: ProPublica Nonprofit Explorer
          </p>
          {results.organizations.length === 0 ? (
            <EmptyState icon={Building} title="No results" description="Try a broader search term or remove the state filter." />
          ) : (
                  ...
                  {results.organizations.map((org) => (
                    <>
                      <TableRow
                        ...
                        onClick={() => setExpanded((prev) => prev === org.ein ? null : org.ein)}
                      >
                        <TableCell>
                          {expanded === org.ein ? <ChevronUp className="h-4 w-4 text-[hsl(var(--muted-foreground))]" /> : <ChevronDown className="h-4 w-4 text-[hsl(var(--muted-foreground))]" />}
                        </TableCell>
                        <TableCell>
                          <div>
                            <span className="font-medium text-sm">{org.name}</span>
                            {org.city && <span className="ml-2 text-xs text-[hsl(var(--muted-foreground))]">{org.city}</span>}
                          </div>
                        </TableCell>
                        <TableCell className="text-sm">{org.state || "—"}</TableCell>
                        <TableCell>
                          {org.ntee_code ? <Badge variant="secondary" className="font-mono text-xs">{org.ntee_code}</Badge> : "—"}
                        </TableCell>
                        <TableCell className="text-right font-mono text-sm">{usd(org.income_amount)}</TableCell>
                        <TableCell className="text-right font-mono text-sm text-[hsl(var(--muted-foreground))]">{usd(org.asset_amount)}</TableCell>
                        <TableCell className="text-right text-sm text-[hsl(var(--muted-foreground))]">
                          {org.form_990_count || "—"}
                          {org.have_pdfs && <ExternalLink className="h-3 w-3 text-[hsl(var(--primary))] inline ml-1" />}
                        </TableCell>
                      </TableRow>
                      {expanded === org.ein && (
